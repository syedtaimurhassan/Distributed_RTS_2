...
def test_run_case_pipeline_writes_expected_artifacts(sample_case_path, tmp_path) -> None:
    ...
    assert (layout.simulation_results_dir / "simulation_frame_hops.csv").exists()
    assert (layout.simulation_results_dir / "simulation_streams.csv").exists()
    assert (layout.analysis_results_dir / "analysis_result.json").exists()
    assert (layout.analysis_results_dir / "analysis_link_breakdown.csv").exists()
    assert (layout.analysis_results_dir / "analysis_streams.csv").exists()
    assert (layout.comparison_results_dir / "comparison_result.json").exists()
    assert (layout.comparison_results_dir / "comparison_streams.csv").exists()
    assert (layout.comparison_results_dir / "comparison_summary.csv").exists()
